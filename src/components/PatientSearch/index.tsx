...
export const PatientSearch = ({onSelectPatient}: {onSelectPatient: (patient: Patient) => void}) => {
  const { register, watch, setValue } = useForm<{search: string}>({defaultValues: { search: '' }});
  ...
  useEffect(() => {
    const fetchPatientData = async () => {
     if(debouncedSearchQuery.length >= 2) {
      try {
        const patients = await request({type: QueryType.SEARCH, value: debouncedSearchQuery}); 
    ...
    <div className="py-4 max-w-lg mx-auto">
      {error && <p>{error}</p>}
        ...
        <ul className="bg-white border border-gray-200 rounded-md shadow-md">
            {showDropdown && patients && patients?.length > 0 && patients.map((patient) => (
                <li 
                    key={patient.id} 
                ...
                >
                    ...
                    <p className="text-sm text-gray-500">ID: {patient.identificationNumber}</p>
                </li>
            ))}
        </ul>
    </div>
